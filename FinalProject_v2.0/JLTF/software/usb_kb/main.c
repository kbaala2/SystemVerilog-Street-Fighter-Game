...
void setKeycode(DWORD keycode)
{
	IOWR_ALTERA_AVALON_PIO_DATA(0x08001170, keycode);
}
int main() {
	BYTE rcode;
	...
	while (1) {
		...
		if (GetUsbTaskState() == USB_STATE_RUNNING) {
			if (!runningdebugflag) {
				...
				setKeycode(kbdbuf.keycode[0] << 24 | kbdbuf.keycode[1] << 16 | kbdbuf.keycode[2] << 8 | kbdbuf.keycode[3]);
