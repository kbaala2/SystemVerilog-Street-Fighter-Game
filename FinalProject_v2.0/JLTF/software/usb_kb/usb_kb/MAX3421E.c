...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	spi_bytes[0] = reg + 2;
	spi_bytes[1] = val;
	ret_code = alt_avalon_spi_command(0x080010a0,0,2,spi_bytes,0,NULL,0);
	if(ret_code < 0){
		printf("Error");
	}
	return;
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(int i = 1; i < nbytes + 1; i++){
		spi_bytes[i] = data[i - 1];
	}
	ret_code = alt_avalon_spi_command(0x080010a0, 0, nbytes + 1, spi_bytes, 0, NULL, 0);
	if(ret_code < 0){
		printf("Error");
	}
//	ret_code = alt_avalon_spi_command(0x080010a0, 0, nbytes, data, 0, NULL, 0);
//	if(ret_code < 0){
//		printf("Error");
//	}
	return data + nbytes;
...
BYTE MAXreg_rd(BYTE reg) {
	...
	alt_u8 spi_write[1];
	spi_write[0] = reg;
	ret_code = alt_avalon_spi_command(0x080010a0,0,1,spi_write,1,spi_read,0);
	if(ret_code < 0){
		printf("Error");
	}
	//printf("%c",spi_read[0]);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	alt_u8 spi_write[1];
	spi_write[0] = reg;
	ret_code = alt_avalon_spi_command(0x080010a0, 0, 1 , spi_write, nbytes, data, 0);
	if(ret_code < 0){
		printf("Error");
	}
	return data + nbytes;
